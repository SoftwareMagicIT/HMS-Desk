...
class PlatformFFI {
  ...
  HelpdeskImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(HelpdeskImpl HelpdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = HelpdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
